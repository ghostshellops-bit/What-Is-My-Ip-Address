...
export default function VPNCheckerPage() {
  ...
  const checkVPN = async () => {
    ...
    try {
      ...
      setData(json);
      toast('VPN check complete', 'success');
    } catch {
      toast('Failed to check VPN', 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    <main className="min-h-screen pt-24 pb-12 px-4 md:px-8 bg-ghost-dark">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-orbitron font-bold text-ghost-cyan">VPN CHECKER</h1>
            <p className="text-gray-400 font-mono text-sm">Check if your VPN is working</p>
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-8 border-ghost-cyan/10">
          <div className="text-center">
            ...
            >
              <RefreshCw size={16} className={loading ? 'animate-spin' : ''} /> {loading ? 'SCANNING...' : 'CHECK VPN'}
            </button>
          </div>
          ...
          {data && (
            <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
              ...
              <div className="glass-card rounded-xl p-4 border-ghost-cyan/10 text-center">
                ...
                <div className="font-bold font-mono text-ghost-cyan">{data.security.fraud_score}/100</div>
              </div>
            </div>
          )}
        </div>
